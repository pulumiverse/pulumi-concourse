...
First, clone this repo with the name of the desired provider in place of `concourse`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-concourse
```

Second, replace references to `concourse` with the name of your provider:
...
The following configuration points are available for the `concourse` provider:

- `concourse:apiKey` (environment: `XYZ_API_KEY`) - the API key for `concourse`
- `concourse:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
